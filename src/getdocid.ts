...
async function getDocIdAsync(displayName: string, apiType: ItemType, gitUrl: string): Promise<string | null> {
    ...
    const memberType = ItemType[apiType];
    ...
    if (apiType === ItemType.constructor || apiType === ItemType.method) {
        ...
        // No parameters.
        if (numParams === 0) {
            const methodOrCtor = xdoc.Member?.find((x: any) =>
        ...
        // With parameters.
        const candidates = xdoc.Member?.filter((x: any) =>
            x.$.MemberName === memberNameToUse &&
            x.MemberType[0] === memberType &&
    ...
    const member = xdoc.Member?.find((x: any) =>
